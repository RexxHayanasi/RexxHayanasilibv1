...
local RexxHayanasilib = {
	...
	Flags = {},
	Tabs = {},
	Icons = loadstring(game:HttpGet("https://raw.githubusercontent.com/RexxHayanasi/RexxHayanasilibv1/refs/heads/main/Icons.lua"))()
}

local ViewportSize = workspace.CurrentCamera.ViewportSize
local UIScale = ViewportSize.Y / 450

local Settings = RexxHayanasilib.Settings
local Flags = RexxHayanasilib.Flags

local SetProps, SetChildren, InsertTheme, Create do
	InsertTheme = function(Instance, Type)
		table.insert(RexxHayanasilib.Instances, {
			Instance = Instance,
			Type = Type
		})
		return Instance
	...
	local function Save(file)
		if readfile and isfile and isfile(file) then
			...
			if type(decode) == "table" then
				if rawget(decode, "UISize") then RexxHayanasilib.Save["UISize"] = decode["UISize"] end
				if rawget(decode, "TabSize") then RexxHayanasilib.Save["TabSize"] = decode["TabSize"] end
				if rawget(decode, "Theme") and VerifyTheme(decode["Theme"]) then RexxHayanasilib.Save["Theme"] = decode["Theme"] end
			end
		end
	end
	
	pcall(Save, "RexxHayanasi library V5.json")
end
...
local Connections, Connection = {}, RexxHayanasilib.Connection do
...
local function VerifyTheme(Theme)
	for name,_ in pairs(RexxHayanasilib.Themes) do
		if name == Theme then
			return true
		end
	end
...
local Theme = RexxHayanasilib.Themes[RexxHayanasilib.Save.Theme]

local function AddEle(Name, Func)
	RexxHayanasilib.Elements[Name] = Func
end

local function Make(Ele, Instance, props, ...)
	local Element = RexxHayanasilib.Elements[Ele](Instance, props, ...)
	return Element
end
...
-- /////////// --
function RexxHayanasilib:GetIcon(index)
...
function RexxHayanasilib:SetTheme(NewTheme)
	if not VerifyTheme(NewTheme) then return end
	
	RexxHayanasilib.Save.Theme = NewTheme
	SaveJson("RexxHayanasi library V5.json", RexxHayanasilib.Save)
	Theme = RexxHayanasilib.Themes[NewTheme]
	
	Comnection:FireConnection("ThemeChanged", NewTheme)
	table.foreach(RexxHayanasilib.Instances, function(_,Val)
...
function RexxHayanasilib:SetScale(NewScale)
	NewScale = ViewportSize.Y / math.clamp(NewScale, 300, 2000)
	UIScale, ScreenGui.Scale.Scale = NewScale, NewScale
end

function RexxHayanasilib:MakeWindow(Configs)
	local WTitle = Configs[1] or Configs.Name or Configs.Title or "RexxHayanasi Library V5"
	local WMiniText = Configs[2] or Configs.SubTitle or "by : RexxHayanasi9999"
	...
	local UISizeX, UISizeY = unpack(RexxHayanasilib.Save.UISize)
	...
	local MainScroll = InsertTheme(Create("ScrollingFrame", Components, {
		Size = UDim2.new(0, RexxHayanasilib.Save.TabSize, 1, -TopBar.Size.Y.Offset),
	...
	ConnectSave(ControlSize1, function()
		if not Minimized then
			RexxHayanasilib.Save.UISize = {MainFrame.Size.X.Offset, MainFrame.Size.Y.Offset}
			SaveJson("RexxHayanasi library V5.json", RexxHayanasilib.Save)
		end
	end)
	
	ConnectSave(ControlSize2, function()
		RexxHayanasilib.Save.TabSize = MainScroll.Size.X.Offset
		SaveJson("RexxHayanasi library V5.json", RexxHayanasilib.Save)
	end)
	...
	function Window:SelectTab(TabSelect)
		if type(TabSelect) == "number" then
			RexxHayanasilib.Tabs[TabSelect].func:Enable()
		else
			for _,Tab in pairs(RexxHayanasilib.Tabs) do
	...
	function Window:MakeTab(paste, Configs)
		...
		TIcon = RexxHayanasilib:GetIcon(TIcon)
		...
		local function Tabs()
			...
			table.foreach(RexxHayanasilib.Tabs, function(_,Tab)
		...
		FirstTab = true
		local Tab = {}
		table.insert(RexxHayanasilib.Tabs, {TabInfo = {Name = TName, Icon = TIcon}, func = Tab, Cont = Container})
		Tab.Cont = Container
		...
		function Tab:AddSection(Configs)
			...
			local Section = {}
			table.insert(RexxHayanasilib.Options, {type = "Section", Name = SectionName, func = Section})
