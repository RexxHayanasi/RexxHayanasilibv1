...
task.spawn(function()
    while task.wait(2) do
        ...
        statsPanel:Set({
            Description = string.format("Ping: %dms | FPS: %d | Memory: %.1fMB", ping, fps, mem)
        })
    end
end)
...
quickAccess:AddButton({
    ...
    Callback = function()
        Window:Notify({
            Title = "System", 
            Description = "Menjalankan script...", 
            Duration = 2 
        })
    end
})
...
quickAccess:AddButton({
    ...
    Callback = function()
        Window:Notify({
            Title = "System", 
            Description = "Membersihkan antarmuka...", 
            Duration = 2 
        })
    end
})
...
Window:AddButton({
    ...
    Callback = function()
        ...
        if success then
            Window:Notify({
                Title = "Berhasil!", 
                Description = "Dead Rails berjalan!", 
                Duration = 3 
            })
        else
            Window:Notify({
...
Window:AddToggle({
    ...
    Callback = function(state)
        Window:Notify({
            Title = "Visuals", 
            Description = state and "ESP Aktif" or "ESP Nonaktif", 
            Duration = 2 
        })
    end
})
